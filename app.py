...
#IPAddress = configD['IPAddress'].rstrip()
#ID = configD['username'].rstrip()
#PWD = configD['password'].rstrip()
g = geocoders.GoogleV3(configD['geoAPIKEY'].rstrip())
#compName = configD['compName'].rstrip()
#dbName = configD['dbName'].rstrip()
#file4D = configD['file4D'].rstrip()
...
def print_rules():
	...
	testsql = "Select * From t_Claim WHERE CAST(DOSFrom1 as date) BETWEEN '"+str(startDate)+"' and '"+str(endDate) + "'"
...
#and isnull(parentclaimid,'')=''	

def rule2():
	print("rule 2")
	rule2sql = "Select DoctorID,DoctorLName,DoctorFName,DoctorGroupID,PracticeID,GroupID,FacilityID,FacName,FacAddress1,DOSFrom1,DOSFrom2,DOSFrom3,DOSFrom4,DOSFrom5,DOSFrom6 From t_Claim WHERE CAST(DOSFrom1 as date) BETWEEN '"+str(startDate)+"' and '"+str(endDate) + "'"
...
def rule3():
	...
	rule3sql = "Select PatientID,DoctorID,DoctorLName,DoctorFName,PracticeID,FacName,Proc1,Proc2,Proc3,Proc4,Proc5,Proc6,DOSFrom1,DOSFrom2,DOSFrom3,DOSFrom4,DOSFrom5,DOSFrom6 From t_Claim WHERE CAST(DOSFrom1 as date) BETWEEN '"+str(startDate)+"' and '"+str(endDate) + "'"
	...
	for index, row in df_rule3.iterrows():
		FacName = df_PractiveLookup.loc[row['PracticeID']].values[0]
	...
	for Patient in PatientList:
		for index, row in df_rule3.iterrows():
			if row['PatientID'] == Patient:
				...
				removelist = []
				for i in Codes:
					if not i.strip() or pd.isnull(i) or not i in CPT_codes:
						removelist.append(count)
					count +=1
	...
	return df_rule3[['Practice Name',"rule3Flag"]]
...
def rule4():
	...
	rule4sql = "Select PracticeID, DoctorID,DoctorLName,DoctorFName, Proc1,Proc2,Proc3,Proc4,Proc5,Proc6,DOSFrom1,DOSFrom2,DOSFrom3,DOSFrom4,DOSFrom5,DOSFrom6,DiagCode1,DiagCode2,DiagCode3,DiagCode4,DiagCode5,DiagCode6 From t_Claim WHERE CAST(DOSFrom1 as date) BETWEEN '"+str(startDate)+"' and '"+str(endDate) + "'"
...
def rule5():
	...
	rule5sql = "Select Proc1,Proc2,Proc3,Proc4,Proc5,Proc6,DoctorID,DoctorLName,DoctorFName,DoctorGroupID,PracticeID,GroupID,FacilityID,FacName,FacAddress1,DOSFrom1,DOSFrom2,DOSFrom3,DOSFrom4,DOSFrom5,DOSFrom6 From t_Claim WHERE CAST(DOSFrom1 as date) BETWEEN '"+str(startDate)+"' and '"+str(endDate) + "'"
	...
	for key,value in CodesDictionlaryofLists.items():
		...
		WholeDictionary[key] = {
			'Specialty': df_SpecialtyLookup.loc[key].values[0],
...
button1 = Button(root, text="Enter", fg="red", command=print_rules)
button1.grid(row=2)
root.mainloop()
